...
```bash
rm out.jsonl
export HF_HOME=/d/develop/huggingface
./.venv/Scripts/semif-score --mode direct --model Qwen/Qwen3.5-4B --revision 851bf6e806efd8d0a36b00ddf55e13ccb7b8cd0a --input examples/decisions.jsonl --output out.jsonl
```

Jev互換サーバーを起動する（http://127.0.0.1:8000/v1/systemone、起動時にウォームアップ済み）：

```bash
export HF_HOME=/d/develop/huggingface
export SEMIF_API_KEY=change-me   # 任意。付けなければ認証なし
./.venv/Scripts/semif-serve --model Qwen/Qwen3.5-4B --revision 851bf6e806efd8d0a36b00ddf55e13ccb7b8cd0a --port 8000
```
